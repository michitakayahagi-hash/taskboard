...
function TaskCard({ task, dragging, members, doneColIds, onPointerDown, onClick, onComplete, onRevert, onComment, onUpdateField }: {
  ...
  return (
    <div onPointerDown={(e) => onPointerDown(e, task)} onClick={() => onClick(task)}
      style={{ background: dragging ? "#f0f0ff" : overdue ? "#fff5f5" : "#fff", borderRadius: 12, padding: "12px 14px", marginBottom: 8, boxShadow: dragging ? "0 0 0 2px #6366f1, 0 8px 24px rgba(99,102,241,.2)" : overdue ? "0 0 0 2px #ef4444, 0 2px 8px rgba(239,68,68,.18)" : "0 1px 4px rgba(99,102,241,.08)", borderLeft: overdue ? "4px solid #ef4444" : `3px solid ${p.color}`, cursor: dragging ? "grabbing" : "grab", opacity: dragging ? 0.4 : 1, transition: "box-shadow .12s,opacity .12s,background .12s", touchAction: "pan-y" }}>
      <p style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 700, color: overdue ? "#b91c1c" : "#1e1b4b", fontFamily: "'Noto Sans JP',sans-serif", lineHeight: 1.4 }}>{overdue && <span style={{ marginRight: 4 }}>🚨</span>}{task.title}</p>
